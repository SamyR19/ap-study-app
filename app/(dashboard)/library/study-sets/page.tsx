...
import { FlashcardSet } from '@/types/study-tools';
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-charcoal">Study Sets</h1>
          <p className="text-charcoal-light">{sets.length} {sets.length === 1 ? 'set' : 'sets'}</p>
        </div>

        <button
          onClick={() => router.push('/create/flashcards')}
      ...
      {filteredSets.length === 0 && !searchQuery && (
        <div className="text-center py-16">
          ...
          <h3 className="text-lg font-semibold text-charcoal mb-2">No study sets yet</h3>
          <p className="text-charcoal-light mb-6">Create your first flashcard set to start studying</p>
          <button
            onClick={() => router.push('/create/flashcards')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSets.map((set, index) => (
            <motion.div
              ...
              onClick={() => router.push(`/library/study-sets/${set.id}`)}
            >
              {/* Card Header */}
              <div className="p-4 pb-0">
                    ...
                    {openMenuId === set.id && (
                      <div className="absolute right-0 top-8 w-40 bg-white rounded-xl shadow-lg border border-cream-200 py-1 z-10">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/library/study-sets/${set.id}/edit`);
                          }}
                ...
                <h3 className="font-semibold text-charcoal line-clamp-2 mb-1">{set.title}</h3>
                {set.description && (
                  <p className="text-sm text-charcoal-light line-clamp-2">{set.description}</p>
                )}
              </div>

              {/* Card Footer */}
              <div className="px-4 py-3 mt-3 border-t border-cream-100 flex items-center justify-between text-sm">
                <span className="text-charcoal-light">{set.card_count} {set.card_count === 1 ? 'card' : 'cards'}</span>
